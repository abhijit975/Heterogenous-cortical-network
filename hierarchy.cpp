...
double jee_in1, jee_out, jei, jie, jii, peeout, peein1,peein2,jee_in2;
int Ncells = ne + ni;
double k = pow(1.0*K,0.5);

jee_out = 90.0*taui/(k*taue);
jee_in1 = jee_out/1.2;
jee_in2 = jee_out/1.8 ;			//uncomment this line and comment the above line to get an unbalanced network with jeein greater than jeeout
jie = 90.0*(pee/pie)*(1./(k));
	...
	for(i = popstart; i<popend; i++)
	{
		for(j = popstart; j<popend; j++)
		{
			if (ran2(&seed)<peein1)
			{
				if(nec[p]<25)
				{
				weights[i][j]= 	jee_out*ratiojee/90.0;
				}
				else{
				weights[i][j] = jee_in1*pee/(nec[p]*peeout);}
	...
	for(i = popstart; i<popend; i++)
	{
		...
		for(kpop = p+1; kpop < ncl; kpop++)
		{
			for(j=counter; j< counter + nec[kpop]; j++)
			{
				...
				{
					if (ran2(&seed)<peein2)
						weights[i][j] = jee_in2*pee/(peeout*nec[kpop]);
				}				
				else
				{					
					if (ran2(&seed)<peeout)
